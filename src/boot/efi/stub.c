...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        /* check the load options: if they don't pass the check, fall back to uki_cmdline */
        if (use_load_options(image, loaded_image, szs[UNIFIED_SECTION_CMDALLW] > 0, &cmdline) &&
            check_cmdline(cmdline, allowlist)) {
                ...
                printf("Command line provided via load options/efi shell does not pass allowlist checks, using the trusted UKI kernel command line stored in .cmdline section\n");
        }
